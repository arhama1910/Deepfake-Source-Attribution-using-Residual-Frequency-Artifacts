...
export default function KineticDotsLoader({ className, variant = 'emerald' }: KineticDotsLoaderProps = {}) {
  const dots = 4; // Increased to 4 for better rhythm
  const isEmerald = variant === 'emerald';

  return (
    <div className={cn('flex items-center justify-center min-h-[160px] p-4 bg-transparent', className)}>
      <div className='flex gap-5'>
        {[...Array(dots)].map((_, i) => (
          ...
          >
            ...
            >
              <div 
                className={cn(
                  ...
                  isEmerald 
                    ? 'bg-gradient-to-b from-emerald-300 via-[#125B4D] to-[#083C33] shadow-[0_0_15px_rgba(13,79,67,0.6)]'
                    : 'bg-gradient-to-b from-cyan-300 to-blue-600 shadow-[0_0_15px_rgba(6,182,212,0.6)]'
                )}
                style={{
            ...
            <div 
               className={cn(
                 'absolute bottom-0 w-10 h-3 rounded-[100%] opacity-0 border',
                 isEmerald ? 'border-emerald-600/30' : 'border-cyan-500/30'
               )}
               style={{
            ...
            <div 
              className={cn(
                'absolute -bottom-1 w-5 h-1.5 rounded-[100%] blur-sm',
                isEmerald ? 'bg-emerald-800/40' : 'bg-cyan-500/40'
              )}
              style={{
