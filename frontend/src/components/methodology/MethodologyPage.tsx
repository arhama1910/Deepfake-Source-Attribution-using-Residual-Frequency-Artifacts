        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {(modelRegistry?.attribution_classes || [
              ...
              const isDiffusion = clsName.toLowerCase().includes('diffusion') || clsName.toLowerCase().includes('midjourney');
              return (
                <div 
                  key={clsName}
                  className="p-5 rounded-[22px] bg-white border border-[#D5D9D1] shadow-xs flex flex-col justify-between space-y-3 hover:border-[#0D5145] hover:shadow-sm transition-all"
                >
                  <div className="flex items-center justify-between text-xs font-mono text-[#52706A]">
                    <span>CLASS {String(idx).padStart(2, '0')}</span>
                    <span className={`w-2 h-2 rounded-full ${isReal ? 'bg-[#0D5145]' : 'bg-[#125B4D]'}`} />
                  </div>
                  <div>
                    <h4 className="text-base font-bold text-[#083C33]">{clsName}</h4>
                    ...
                    <p className="text-[11px] text-[#52706A] mt-1 leading-normal">
                      {isDiffusion
                        ? 'Spectral power falloff at high frequencies'
                        : isReal
                        ? 'Continuous PRNU silicon noise floor'
                        : 'Periodic transposed conv checkerboard grid'}
                    </p>
                  </div>
                </div>
              );
